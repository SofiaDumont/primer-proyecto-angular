...
export class EstacionamientoService {
  ...
  estacionarAuto(patenteAuto: string, idCochera: number): Promise<Estacionamiento> {
  ...
  cobrarEstacionamiento(idCochera: number, patente: string, costo: number): Promise<any> {
    return fetch('http://localhost:4000/estacionamientos/cerrar', {
      method: 'PATCH',
      headers: {
        Authorization: "Bearer " + this.auth.getToken(),
